...
int graphColoring(Grafo_Matriz *gr, int m) {
    int i;
...
int printaMapa(int **mapa, int num_vertices, int rem) {
    int i, j;
    printf("MAPA %d\n", rem);
    for(i=0; i < num_vertices; i++){
        for(j=0; j < num_vertices; j++)
...
void zykovAdiciona(int **mapa, int vertice_atual, int v1, int num_vertices) {
    int i, j, acres = 0;
    for(i=0; i < num_vertices; i++){
        if(mapa[v1][i] && mapa[vertice_atual][i] == 0){
            for(j=0; j < num_vertices; j++){
                if(mapa[i][j]){
                    acres++;
                }
        ...
        if(acres == 1){
            for(j=0; j < num_vertices; j++){
                if(mapa[v1][j] == 1 && mapa[vertice_atual][j] == 0){
                    mapa[vertice_atual][j] = 1;
                }
            }
        }
        acres = 0;
    ...
    if(verificaClique(mapa, num_vertices)){
        for(i=0; i < num_vertices; i++){
            if(mapa[vertice_atual][i] == 0 && vertice_atual != i){
                //printaMapa(mapa, num_vertices, i);
                //zykovAdiciona(mapa, vertice_atual, i, num_vertices);
